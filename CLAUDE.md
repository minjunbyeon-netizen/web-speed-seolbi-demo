...
- 모바일 컴팩트(2026-06-22): ≤680px 전용 오버라이드 2블록(섹션 패딩·갭·히어로 높이 축소 / 작업과정·네이버리뷰). PC 무영향.
  - **작업과정**: 카드 → 한 줄 행(번호 34px + 제목·설명 grid). 4스텝 세로 ~60% 단축.
  - **9곳 한마디 = 폰에서 네이버 리뷰 스타일**: 이미지오버레이 카드 대신 리뷰 레이아웃. `.vc-review`(데스크톱 `display:none`) = 프로필 아바타(지역 첫 글자·tone별 블루)+닉네임(`{지역} 김**`)+별점(★)+날짜. 본문(order:1) 아래 사진 썸네일(order:2, height 152px, 다크오버레이·지역명 제거, 로딩 플레이스홀더 `--line`). 키워드칩 = 서비스2 + **네이버식 감정키워드1**(친절해요·시간약속·꼼꼼해요 등 카드별 상이), pill+체크(✓ CSS 보더). 닉네임·별점·날짜·키워드 전부 **가상**.
  - **별점 분포(2026-06-22 다양화)**: 7×5점 + 2×4점(삼방동·강서구, 5번째 별 `.s-off` 흐림) = 평균 4.78 → 신뢰밴드 표기 `4.8 (후기 9)`로 동기화. 4점 카드 늘리면 평균·표기 같이 조정.
  - **썸네일 eager 로딩**: 첫 3개 카드(명지동·하단동·삼방동) `loading="eager"`(명지동은 `fetchpriority="high"`), 나머지 `lazy`. 전 카드 `decoding="async"`. 갤러리 진입 시 첫 화면 썸네일 즉시 표시 목적.
...
- **FOMO 가상**: 히어로 "잔여 3팀 마감 임박" 희소성 배지 + 좌하단 실시간 출동 토스트(`#liveToast`, main.js `feed` 배열 9건 순환, 9초 간격) + 누적 카운터(`#liveCount`, 43건 시작·토스트당 +1). 전부 **가상 데이터**(실제 출동 현황 아님). 실서비스 전 제거 또는 실데이터 연동 필요.
- 섹션: 톱바 · 헤더 · 히어로(풀블리드) · 마퀴 · 신뢰지표 · 서비스6(이미지) · 장비/강점 · 작업과정 · **비용 안내+안심약속(PRICING)** · **시공사례=현장 한마디 9곳(writeplz `9곳의 한 마디` 패턴 이식, 현장사진 배경+다크 오버레이(rgba 8,28,44,0.55)+지역명)** · 서비스지역 · **신뢰밴드(네이버 평점/블로그·플레이스 링크)** · 견적문의 · 푸터
