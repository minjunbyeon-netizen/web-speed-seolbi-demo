...
- 모바일 컴팩트(2026-06-22): ≤680px 전용 오버라이드 2블록(섹션 패딩·갭·히어로 높이 축소 / 작업과정·네이버리뷰). PC 무영향.
  ...
  - **9곳 한마디 = 폰에서 네이버 리뷰 스타일**: 이미지오버레이 카드 대신 리뷰 레이아웃. `.vc-review`(데스크톱 `display:none`) = 프로필 아바타(지역 첫 글자·tone별 블루)+닉네임(`{지역} 김**`)+별점(★, 8×5+1×4=평균 4.9 일치)+날짜. 본문(order:1) 아래 사진 썸네일(order:2, height 152px, 다크오버레이·지역명 제거). 키워드칩은 pill+체크(✓ CSS 보더). 닉네임·별점·날짜 전부 **가상**.
